...
// Determine backend API URL (resolves relative if running on Render/localhost, or absolute Render URL if running on GitHub Pages)
const PREDICT_API_URL = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' || window.location.hostname.includes('onrender.com')
  ? './api/predict'
  : 'https://code-alpha-iris-classification-2.onrender.com/api/predict';
